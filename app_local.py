...
# BM25 + Rerank (if you want lexical + semantic combos)
...
################################################################################
# 1. PDF -> High-DPI Images
################################################################################
def pdf_to_images(pdf_bytes, dpi=400):
    """
    Converts each page in a PDF (in memory) to a PIL Image at the specified DPI.
...
################################################################################
# 2. Async Gemini Call per Page
################################################################################
async def analyze_image_gemini(img: Image, prompt: str):
    """
    Sends (img + prompt) to Gemini, returns the raw text or JSON.
    """
    ...
    # Because Streamlit runs in an event loop, wrap the synchronous call
...
def create_ensemble_retriever(docs, vectorstore):
    ...
    final_retriever = ContextualCompressionRetriever(base_compressor=compressor, base_retriever=ensemble)
...
################################################################################
def main():
    st.title("PDF + Gemini + Vector Search Demo")

    # Hard-coded Gemini prompt (the user won't see/edit this)
    ...
    # Step B: Once PDF is uploaded, show "Process PDF" button
    if st.button("Process PDF"):
        # 1) Convert PDF to images
        with st.spinner("Converting PDF to images at high DPI..."):
            pdf_bytes = uploaded_pdf.read()
            images = pdf_to_images(pdf_bytes, dpi=400)
        st.write(f"PDF has {len(images)} pages.")

        # 2) Asynchronously call Gemini on each page
        ...
        st.success("Processing complete. You can now query below!")

    # Step C: Query interface
    if "retriever" in st.session_state and st.session_state["retriever"]:
        query = st.text_input("Enter your query to search Gemini's JSON results:")
        if query:
            st.write("Searching...")
